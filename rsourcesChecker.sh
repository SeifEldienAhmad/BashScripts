...
echo "$(date '+%Y-%m-%d  %H:%M:%S') - CPU: ${CPU}% - RAM: ${RAM}%" >> "$HOME/Desktop/Scripts/resourcesChecker/usageLog.txt"

# Check if CPU usage exceeds 80%, and print a warning if it does
if [ $(echo "$CPU > 80" | bc -l) -eq 1 ]
then
    echo "High CPU usage: $CPU%" | mail -s "CPU Alert from $(hostname)" seifahmed7293@gmail.com
...
then
    echo "High RAM usage: $RAM%" | mail -s "RAM Alert from $(hostname)" seifahmed7293@gmail.com
